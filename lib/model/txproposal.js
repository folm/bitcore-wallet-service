...
TxProposal.create = function(opts) {
  ...
  x.outputs = _.map(opts.outputs, function(output) {
    return _.pick(output, ['amount', 'toAddress', 'message', 'script', 'anondest']);
  });
  ...
  x.status = 'temporary';
  x.actions = [];
  x.anondest = x.outputs[0].anondest || ''; //@TODO check if this is correct or if we take it from the outputs object
...
TxProposal.prototype._addSignaturesToBitcoreTx = function(tx, signatures, xpub) {
  ...
  _.each(signatures, function(signatureHex) {
    ...
    try {
      ...
      console.log('each signature error', e);
    };
  });

  console.log('i, tx', i, tx);
